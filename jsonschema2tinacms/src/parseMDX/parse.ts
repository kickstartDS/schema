// https://github.com/tinacms/tinacms/blob/e74d1dd8496672a066c5849101e327742013ab91/packages/%40tinacms/graphql/src/mdx/parse.ts
...
import type { GraphQLConfig, RichTypeInner } from '@tinacms/graphql/dist/types'
...
export const parseMDX = (
  ...
  const tree = unified().use(markdown as any).use(mdx as any).parse(value)
...
const parseMDXInner = (
    ...
    node.attributes.forEach((attribute: any) => {
      if (attribute.type === 'mdxJsxAttribute') {
        ...
        if (!template) {
          ...
          const field = template.fields.find((field) => field.name === attribute.name)
          if (!field) {
  ...
  const slateTree = tree.children.map((node: MdxAstNode) => remarkToSlate(node, graphQLconfig, schema))
    ...
    case 'string':
      if (field.list) {
        ...
        if (attribute.value?.type === 'mdxJsxAttributeValueExpression') {
          attribute.value.data.estree.body.forEach((item: any) => {
            if (item.type === 'ExpressionStatement') {
              if (item.expression.type === 'ArrayExpression') {
                props[field.name] = item.expression.elements.map((element: any) => {
          ...
          attribute.value.data.estree.body.forEach((item: any) => {
            if (item.type === 'ExpressionStatement') {
              ...
              if (item.expression.type === 'ObjectExpression') {
                const objectProps = {}
                item.expression.properties.forEach((property: any) => {
                  ...
                  if (field.fields) {
                    ...
                    field.fields.forEach((field) => {
                      parseField(property, field, objectProps, graphQLconfig, schema)
                    })
                  }
                })
                props[field.name] = objectProps
              } else if (item.expression.type === 'ArrayExpression') {
                const objectArrayProps: any[] = []
                item.expression.elements.forEach((element: any) => {
...
function remarkToSlate(node: MdxAstNode, graphQLconfig: GraphQLConfig, schema: TinaCloudSchemaEnriched): any {
  const types = {
    ...defaultNodeTypes,
    heading: {
  ...
  switch (node.type) {
    case 'heading':
      return {
        ...
        children: node.children.map((node) => remarkToSlate(node, graphQLconfig, schema)),
      }
    case 'list':
      return {
      ...
      node.children.forEach((child) => {
        if (child.type === 'list') {
          realChildren.push({
            ...
            children: child.children.map((node) => remarkToSlate(node, graphQLconfig, schema)),
          })
        } else {
          realChildren.push({
