import { capitalize } from '@kickstartds/jsonschema-utils';

import { INetlifyCmsConfig, INetlifyCmsCollection, INetlifyCmsField } from './@types/index.js';

const defaultConfig: INetlifyCmsConfig = {
  backend: {
    name: 'git-gateway',
    branch: 'master'
  },
...
export function createConfig(
  contentFields: INetlifyCmsField[],
  settingsFields: INetlifyCmsField[],
  baseConfig: INetlifyCmsConfig = defaultConfig,
  collectionName: string = 'pages',
  settingsName: string = 'settings'
): INetlifyCmsConfig {
  const sortable = (field: INetlifyCmsField): boolean => field.widget === 'object' || field.widget === 'list';

  const sortFields = (contentFieldA: INetlifyCmsField, contentFieldB: INetlifyCmsField): number => {
  ...
  const sortFieldsDeep = (fields: INetlifyCmsField[]): INetlifyCmsField[] => {
  ...
  const pages: INetlifyCmsCollection = {
    name: collectionName,
  ...
  const settings: INetlifyCmsCollection = {
    name: settingsName,
  ...
  const pagesCollection = baseConfig.collections.find((collection) => collection.name === collectionName);
  if (pagesCollection) {
  ...
  const settingsCollection = baseConfig.collections.find((collection) => collection.name === settingsName);
