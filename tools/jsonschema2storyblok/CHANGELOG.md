# Change Log - @kickstartds/jsonschema2storyblok

This log was last generated on Mon, 09 Mar 2026 14:37:38 GMT and should not be manually modified.

## 1.6.10
Mon, 09 Mar 2026 14:37:38 GMT

### Patches

- rework group handling

## 1.6.9
Mon, 28 Jul 2025 13:22:55 GMT

_Version update only_

## 1.6.8
Thu, 24 Jul 2025 13:17:05 GMT

_Version update only_

## 1.6.7
Tue, 08 Jul 2025 10:57:31 GMT

### Patches

- update type annotations
- update layering in utils

## 1.6.6
Sat, 05 Jul 2025 15:22:43 GMT

_Version update only_

## 1.6.5
Fri, 04 Jul 2025 20:35:48 GMT

_Version update only_

## 1.6.4
Fri, 04 Jul 2025 15:47:19 GMT

_Version update only_

## 1.6.3
Wed, 02 Jul 2025 09:04:33 GMT

_Version update only_

## 1.6.2
Thu, 31 Oct 2024 14:24:58 GMT

### Patches

- add options to icon custom field plugin

## 1.6.1
Fri, 25 Oct 2024 09:36:32 GMT

### Patches

- add preview template to ref arrays

## 1.6.0
Fri, 25 Oct 2024 07:12:38 GMT

### Minor changes

- add poc for preview annotation support

## 1.5.40
Thu, 24 Oct 2024 15:56:08 GMT

### Patches

- use correct real_name in refArrays

## 1.5.39
Thu, 24 Oct 2024 14:45:17 GMT

### Patches

- add maximum to reffed bloks entries

## 1.5.38
Mon, 14 Oct 2024 17:11:43 GMT

### Patches

- add icon capabilities to Storyblok

## 1.5.37
Thu, 10 Oct 2024 15:00:47 GMT

### Patches

- remove console log

## 1.5.36
Mon, 30 Sep 2024 11:39:44 GMT

### Patches

- add additional cms format, add table format

## 1.5.35
Wed, 11 Sep 2024 12:34:08 GMT

_Version update only_

## 1.5.34
Tue, 20 Aug 2024 13:12:42 GMT

_Version update only_

## 1.5.33
Wed, 10 Jul 2024 12:46:32 GMT

_Version update only_

## 1.5.32
Tue, 09 Jul 2024 11:13:35 GMT

_Version update only_

## 1.5.31
Tue, 09 Jul 2024 10:38:59 GMT

_Version update only_

## 1.5.30
Fri, 05 Jul 2024 14:49:28 GMT

_Version update only_

## 1.5.29
Fri, 28 Jun 2024 15:41:22 GMT

_Version update only_

## 1.5.28
Tue, 25 Jun 2024 10:40:59 GMT

_Version update only_

## 1.5.27
Tue, 25 Jun 2024 10:33:43 GMT

_Version update only_

## 1.5.26
Tue, 25 Jun 2024 09:46:26 GMT

_Version update only_

## 1.5.25
Mon, 24 Jun 2024 10:40:19 GMT

_Version update only_

## 1.5.24
Wed, 17 Apr 2024 18:29:14 GMT

_Version update only_

## 1.5.23
Wed, 17 Apr 2024 13:59:55 GMT

_Version update only_

## 1.5.22
Mon, 15 Apr 2024 13:24:51 GMT

_Version update only_

## 1.5.21
Mon, 15 Apr 2024 13:21:12 GMT

_Version update only_

## 1.5.20
Thu, 11 Apr 2024 08:42:25 GMT

### Patches

- fix nestable attribute for components

## 1.5.19
Wed, 20 Mar 2024 20:13:09 GMT

_Version update only_

## 1.5.18
Wed, 20 Mar 2024 14:13:07 GMT

_Version update only_

## 1.5.17
Mon, 18 Mar 2024 16:21:35 GMT

_Version update only_

## 1.5.16
Mon, 18 Mar 2024 16:16:53 GMT

_Version update only_

## 1.5.15
Mon, 18 Mar 2024 15:09:15 GMT

_Version update only_

## 1.5.14
Mon, 18 Mar 2024 13:53:57 GMT

_Version update only_

## 1.5.13
Mon, 18 Mar 2024 10:56:30 GMT

_Version update only_

## 1.5.12
Thu, 14 Mar 2024 14:00:48 GMT

_Version update only_

## 1.5.11
Thu, 14 Mar 2024 13:50:59 GMT

_Version update only_

## 1.5.10
Thu, 14 Mar 2024 13:15:38 GMT

_Version update only_

## 1.5.9
Thu, 14 Mar 2024 12:45:31 GMT

_Version update only_

## 1.5.8
Thu, 14 Mar 2024 12:11:35 GMT

_Version update only_

## 1.5.7
Sun, 11 Feb 2024 10:59:10 GMT

### Patches

- remove unneeded dependencies

## 1.5.6
Sun, 04 Feb 2024 16:52:43 GMT

### Patches

- add incrementing ids to components

## 1.5.5
Tue, 30 Jan 2024 15:51:13 GMT

### Patches

- fix handling of string arrays

## 1.5.4
Mon, 29 Jan 2024 19:29:51 GMT

### Patches

- trigger another release without log statement

## 1.5.3
Mon, 29 Jan 2024 17:54:39 GMT

### Patches

- clean up component grouping with classification
- clean up component grouping with classification

## 1.5.2
Mon, 29 Jan 2024 13:41:37 GMT

### Patches

- correctly add reffed storyblok components to group
- fine-tune controls for basic types

## 1.5.1
Tue, 23 Jan 2024 11:35:50 GMT

### Patches

- fix restriction setting for component whitelists

## 1.5.0
Tue, 23 Jan 2024 10:56:23 GMT

### Minor changes

- improve handling of refs

## 1.4.3
Sun, 21 Jan 2024 22:51:15 GMT

### Patches

- remove logging from published release

## 1.4.2
Sun, 21 Jan 2024 22:45:57 GMT

### Patches

- fix naming for refs in storyblok converter

## 1.4.1
Sun, 21 Jan 2024 22:30:52 GMT

### Patches

- fix conversion of objects in arrays

## 1.4.0
Fri, 19 Jan 2024 10:59:21 GMT

### Minor changes

- fix naming of reffed schemas (singular and in arrays)

## 1.3.1
Mon, 15 Jan 2024 13:01:48 GMT

_Version update only_

## 1.3.0
Wed, 10 Jan 2024 08:06:43 GMT

### Minor changes

- update to streamlined reducer

## 1.2.1
Mon, 04 Dec 2023 14:09:19 GMT

### Patches

- segment component types

## 1.2.0
Mon, 04 Dec 2023 11:43:19 GMT

### Minor changes

- switch to reworked utils and reducer

## 1.1.0
Thu, 23 Nov 2023 21:05:30 GMT

### Minor changes

- update to reworked reducer

## 1.0.25
Thu, 02 Nov 2023 09:42:54 GMT

### Patches

- Merge different bloks correctly

## 1.0.24
Wed, 01 Nov 2023 23:02:38 GMT

### Patches

- Fix uniqueness filter for elements

## 1.0.23
Wed, 01 Nov 2023 22:21:00 GMT

### Patches

- Filter to unique elements in converter

## 1.0.22
Wed, 01 Nov 2023 22:17:41 GMT

### Patches

- Undo prefixing

## 1.0.21
Wed, 01 Nov 2023 19:47:53 GMT

### Patches

- Update slicing of Storyblok bloks

## 1.0.20
Sun, 29 Oct 2023 11:05:45 GMT

_Version update only_

## 1.0.19
Thu, 26 Oct 2023 21:35:49 GMT

### Patches

- re-export used types

## 1.0.18
Thu, 26 Oct 2023 21:26:21 GMT

_Version update only_

## 1.0.17
Wed, 18 Oct 2023 09:12:21 GMT

### Patches

- Remove debug log statement

## 1.0.16
Tue, 17 Oct 2023 20:29:19 GMT

_Version update only_

## 1.0.15
Sun, 15 Oct 2023 10:08:47 GMT

_Version update only_

## 1.0.14
Thu, 12 Oct 2023 12:44:39 GMT

_Version update only_

## 1.0.13
Wed, 11 Oct 2023 20:57:39 GMT

### Patches

- Fix additional array case

## 1.0.12
Wed, 11 Oct 2023 11:51:20 GMT

_Version update only_

## 1.0.11
Sat, 07 Oct 2023 18:54:59 GMT

_Version update only_

## 1.0.10
Fri, 06 Oct 2023 13:43:55 GMT

### Patches

- update used utils version

## 1.0.9
Fri, 06 Oct 2023 13:08:52 GMT

_Version update only_

## 1.0.8
Thu, 05 Oct 2023 10:51:41 GMT

### Patches

- handle more array cases in reducer

## 1.0.7
Tue, 03 Oct 2023 13:03:14 GMT

_Version update only_

## 1.0.6
Tue, 03 Oct 2023 12:46:28 GMT

_Version update only_

## 1.0.5
Mon, 02 Oct 2023 09:50:58 GMT

_Version update only_

## 1.0.4
Mon, 25 Sep 2023 14:22:48 GMT

_Version update only_

## 1.0.3
Mon, 25 Sep 2023 13:21:10 GMT

_Version update only_

## 1.0.2
Sun, 24 Sep 2023 13:58:19 GMT

_Initial release_

