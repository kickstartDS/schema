...
export function generateNetlifyCMS(
  ...
  const storyblokFields = convertToStoryblok({
    schemaIds,
    ajv
  });

  const configStringStoryblok = JSON.stringify({ components: storyblokFields }, null, 2);
  writeFileSync('dist/components.123456.json', configStringStoryblok);
}
