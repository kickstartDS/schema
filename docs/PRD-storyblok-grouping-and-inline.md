...
### 1.2 Storyblok grouping primitives differ from Stackbit

Storyblok offers three native grouping primitives:

1. **`type: tab`** — top-level horizontal tabs in a blok form. Already auto-generated for nested non-component objects ([storyblok/index.ts L307-L325](../tools/jsonschema2storyblok/src/index.ts#L307)).
2. **`type: section`** — collapsible labeled section inside a tab; references members via a `keys[]` array.
...
The natural mapping for Stackbit's `fieldGroups` (flat, named, icon-bearing groupings inside a single model) is **`section`**. Storyblok's `section` element type is already declared on `IStoryblokSchemaElement` ([@types/index.ts L23, L73-L74](../tools/jsonschema2storyblok/src/@types/index.ts#L23)) but is never emitted.
...
| # | Goal | Measured by |
|---|------|-------------|
| G1 | Storyblok converter emits `type: section` elements driven by `x-cms-group-name`/`-title` | Annotated sibling fields appear under a labeled section in the generated blok schema |
| G2 | Storyblok converter implements `x-cms-group-inline` with key prefixing identical to Stackbit | Wrapped object's fields appear at parent level with `<wrapper>__<child>` keys |
| G3 | A new utility in `@kickstartds/jsonschema-utils` (or a sibling tool) generates Cambria lenses describing every key-mutating transform applied to a schema | Given an annotated schema, a `.lens.yml` file is produced |
| G4 | Generated lenses round-trip: forward transforms un-annotated content into CMS shape; reverse transforms CMS content back | Property-based tests confirm `reverseLens(forward) ∘ forward = id` on representative documents |
| G5 | Stackbit converter produces the same lens output (for `x-cms-group-inline`) so renderers built on the un-annotated base can consume Stackbit data too | Same lens generator, exercised by Stackbit examples, produces working inverse |
...
|---|---|---|---|
| `x-cms-group-name` | Field's `section` membership (via `keys[]`) | No | No |
| `x-cms-group-title` | Section element's `display_name` | No | No |
| `x-cms-group-icon` | Best-effort: section `description` or dropped | No | No |
| `x-cms-group-inline` | Splice children into parent, prefix with `<wrapper>__` | **Yes** | **Yes** |

Only `x-cms-group-inline` requires lens generation. The other three are added to the converter as pure UI metadata.
...
Inside `createBlokSchema`, after sort/inline but before `assignPositions`:

1. Walk fields, bucket by non-`INLINE__` `group` value.
2. For each bucket, emit one `IStoryblokSchemaElement` of `type: 'section'`:
   - `key`: `section-<groupName>` or stable uuid
   ...
   - `keys`: member keys in their already-sorted order
   - `description`: optionally derived from `x-cms-group-icon`
3. Insert the section element at the position of its first member.
4. Strip `group` from members so it doesn't leak into output.

`assignPositions()` requires no change — it iterates `Object.values()` in insertion order and sections will already sit in the right slot.
...
### Phase 1: Storyblok grouping primitives (no key mutation)
...
3. Synthesize `type: section` elements inside `createBlokSchema`, threading `x-cms-group-title` through.
4. Add an example in `examples/storyblok/resources/` exercising sections.
...
- [ ] Storyblok converter emits `type: section` elements when `x-cms-group-name` is present
