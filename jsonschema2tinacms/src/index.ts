...
export default function convert({ jsonSchema, definitions, ajv, configLocation, collectionName = 'pages' }: ConvertParams): string {
  ...
  const configString = `${configDisclaimer}${JSON.stringify(createConfig((contentFields[0] as ObjectType<false>).fields as TinaFieldInner<false>[], baseConfig, collectionName), null, 2)}`;
